...
public class FacebookManager {
    ...
    public FacebookManager(Context context,
            Session.StatusCallback statusCallback, Bundle savedInstanceState) {
        this.context = context;
        initSession(statusCallback, savedInstanceState);
    }

    private void initSession(Session.StatusCallback statusCallback,
    ...
    public void printHashKey(String appPackage) {
        try {
            PackageInfo info = context.getPackageManager().getPackageInfo(
                    appPackage, PackageManager.GET_SIGNATURES);
    ...
    public void getFriendList(Request.GraphUserListCallback callback) {
        Session session = Session.getActiveSession();
        if (session.isOpened()) {
            Request myFriendsRequest = Request.newMyFriendsRequest(session,
                    callback);
...
//             Request shareRequest = Request.newUploadPhotoRequest(Session.getActiveSession(), BitmapFactory.decodeResource(context.getResources(),
...
//			Request shareRequest = Request.newRestRequest(Session.getActiveSession(),
...
//			});
            ...
            Request shareRequest = new Request(session, "me/feed", params,
    ...
    public void getUserImg(boolean allowCachedResponse,
            ImageRequest.Callback callback, String profileId) {
        try {
            ImageRequest.Builder requestBuilder = new ImageRequest.Builder(
                    context, ImageRequest.getProfilePictureUrl(profileId,
