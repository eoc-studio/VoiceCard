...
import eoc.studio.voicecard.facebook.utils.Utils;

/**
 * A class that helps control the audience on Facebook that can see a post made
 * by an app on behalf of a user. Reference:
...
public class Privacy
{
	 ...
	 * and is ultimately governed by the privacy ceiling a user has configured
	 * for an app.<br/>
	...
	private void ValidateListsAccessRequest()
	{
		if (mPrivacySetting != PrivacySettings.CUSTOM)
			throw new UnsupportedOperationException(
					"Can't add / delete from allowed / denied lists when privacy setting is different than \"CUSTOM\"");
	}
	...
	/**
	 * Add a predefined friend list to the allowed list This can <b>only</b> be
	 * ALL_FRIENDS or FRIENDS_OF_FRIENDS to include all members of those sets.
	 * 
	 * @param friendList
	 *            ALL_FRIENDS or FRIENDS_OF_FRIENDS to include all members of
	 *            those sets
	 */
	public void addallowedUserOrListID(PrivacySettings friendList)
	{
		ValidateListsAccessRequest();
		if (friendList != PrivacySettings.ALL_FRIENDS
				|| friendList != PrivacySettings.FRIENDS_OF_FRIENDS)
			throw new UnsupportedOperationException(
					"Can't add this predefined friend list. Only allowed are: ALL_FRIENDS or FRIENDS_OF_FRIENDS");
