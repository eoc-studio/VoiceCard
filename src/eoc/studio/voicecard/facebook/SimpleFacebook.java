...
import eoc.studio.voicecard.facebook.entities.Album;
import eoc.studio.voicecard.facebook.entities.Checkins;
import eoc.studio.voicecard.facebook.entities.Feed;
import eoc.studio.voicecard.facebook.entities.Photo;
import eoc.studio.voicecard.facebook.entities.Profile;
import eoc.studio.voicecard.facebook.entities.Story;
import eoc.studio.voicecard.facebook.entities.Video;
import eoc.studio.voicecard.facebook.utils.Errors;
import eoc.studio.voicecard.facebook.utils.Errors.ErrorMsg;
import eoc.studio.voicecard.facebook.utils.Logger;
...
public class SimpleFacebook
{
	...
	private static SimpleFacebookConfiguration mConfiguration = new SimpleFacebookConfiguration.Builder()
			.build();
	...
	public void login(OnLoginListener onLoginListener)
	{
		...
		else
		{
			...
			{
				session = new Session.Builder(mActivity.getApplicationContext()).setApplicationId(
						mConfiguration.getAppId()).build();
	 ...
	 * // create properties
	 * Properties properties = new Properties.Builder().add(Properties.ID).add(Properties.FIRST_NAME)
	 * 		.add(Properties.PICTURE, attributes).build();
	 * </pre>
	 */
	public void getProfile(Properties properties,
			final OnProfileRequestListener onProfileRequestListener)
	{
		// if we are logged in
		if (isLogin())
		{
	 ...
	 * pictureAttributes.setWidth(500);
	 * // create properties
	 * Properties properties = new Properties.Builder().add(Properties.ID).add(Properties.LAST_NAME)
	 * 		.add(Properties.PICTURE, attributes).add(Properties.BIRTHDAY).build();
	 * </pre>
	 */
	public void getFriends(Properties properties,
			final OnFriendsRequestListener onFriendsRequestListener)
	{
		// if we are logged in
		if (isLogin())
		{
			...
			Request request = new Request(session, "me/friends", bundle, HttpMethod.GET,
					new Request.Callback()
					{
						...
						public void onCompleted(Response response)
						{
							List<GraphUser> graphUsers = typedListFromResponse(response,
									GraphUser.class);
							FacebookRequestError error = response.getError();
							if (error != null)
							{
	...
	public void getAlbums(final OnAlbumsRequestListener onAlbumsRequestListener)
	{
		...
		if (isLogin())
		{
			...
			Request request = new Request(session, "me/albums", bundle, HttpMethod.GET,
					new Request.Callback()
					{
						@Override
						public void onCompleted(Response response)
						{
							List<GraphObject> graphObjects = typedListFromResponse(response,
									GraphObject.class);
							FacebookRequestError error = response.getError();
							if (error != null)
							{
	...
	public void getCheckins(String userId, final OnCheckinsRequestListener onCheckinsRequestListener)
	{
		...
		if (isLogin())
		{
			// move these params to method call parameters
			Session session = getOpenSession();
			Request request = new Request(session, userId + "/checkins", null, HttpMethod.GET,
					new Request.Callback()
					{
						@Override
						public void onCompleted(Response response)
						{
							...
							else
							{
								...
								if (onCheckinsRequestListener != null)
								{
									List<Checkins> checkins = new ArrayList<Checkins>(
											graphObjects.size());
									for (GraphObject graphObject : graphObjects)
									{
	...
	public void getAppRequests(final OnAppRequestsListener onAppRequestsListener)
	{
		...
		if (isLogin())
		{
			Session session = getOpenSession();
			Bundle bundle = null;
			Request request = new Request(session, "me/apprequests", bundle, HttpMethod.GET,
					new Request.Callback()
					{
						@Override
						public void onCompleted(Response response)
						{
							...
							else
							{
								...
								else
								{
									// log
									logError(
											"The GraphObject in Response of getAppRequests has null value. Response="
	...
	public void deleteRequest(String inRequestId,
			final OnDeleteRequestListener onDeleteRequestListener)
	{
		if (isLogin())
		{
			...
			Session session = getOpenSession();
			Request request = new Request(session, inRequestId, null, HttpMethod.DELETE,
					new Request.Callback()
					{
						@Override
						public void onCompleted(Response response)
	...
	public void getScores(final OnScoresRequestListener onScoresRequestListener)
	{
		...
		if (isLogin())
		{
			Session session = getOpenSession();
			Bundle bundle = null;
			Request request = new Request(session, mConfiguration.getAppId() + "/scores", bundle,
						...
						public void onCompleted(Response response)
						{
							...
							else
							{
								...
								else
								{
									// log
									logError(
											"The GraphObject in Response of getScores has null value. Response="
													+ response.toString(), null);
								}
							}
						}
	...
	public void postScore(int score, final OnPostScoreListener onPostScoreListener)
	{
		if (isLogin())
		{
			...
			if (mConfiguration.getPublishPermissions().contains(
					Permissions.PUBLISH_ACTION.getValue()))
			{
				// callback with 'thinking'
				...
				if (!getOpenSessionPermissions().contains(Permissions.PUBLISH_ACTION.getValue()))
				{
					String reason = Errors.getError(ErrorMsg.CANCEL_PERMISSIONS_PUBLISH,
							String.valueOf(mConfiguration.getPublishPermissions()));
					logError(reason, null);
					// callback with 'fail' due to not being logged in
					if (onPostScoreListener != null)
					{
			...
			Request request = new Request(getOpenSession(), "me/scores", param, HttpMethod.POST,
					new Request.Callback()
					{
						@Override
						public void onCompleted(Response response)
	 ...
	 */
	public void publish(final Photo photo, final String albumId,
			final OnPublishListener onPublishListener)
	{
		if (isLogin())
		{
			// if we defined the publish permission
			if (mConfiguration.getPublishPermissions().contains(
					Permissions.PUBLISH_STREAM.getValue()))
			{
				// callback with 'thinking'
				if (onPublishListener != null)
				{
	 ...
	 */
	public void invite(String[] suggestedFriends, String message,
			final OnInviteListener onInviteListener)
	{
		if (isLogin())
		{
			Bundle params = new Bundle();
	...
	public boolean onActivityResult(Activity activity, int requestCode, int resultCode, Intent data)
	{
		...
		{
			return Session.getActiveSession().onActivityResult(activity, requestCode, resultCode,
					data);
		}
		else
		{
	...
	public boolean isLogin()
	{
		...
		if (session == null)
		{
			if (mActivity == null)
			{
				...
				// been initialized
				// This is now possible because the library can be started
				// without context.
				return false;
			}
			session = new Session.Builder(mActivity.getApplicationContext()).setApplicationId(
		...
		/*
		 * Check if we can reload the session when it will be neccesary. We
		 * won't do it now.
		 */
	...
	{
		Session session = getOpenSession();
		Request request = new Request(session, "me/feed", feed.getBundle(), HttpMethod.POST,
				new Request.Callback()
				{
					@Override
					public void onCompleted(Response response)
					{
						...
						else
						{
							// log
							logError(
									"The GraphObject in Response of publish action has null value. Response="
											+ response.toString(), null);
							if (onPublishListener != null)
							{
	...
	private static void publishImpl(Story story, final OnPublishListener onPublishListener)
	{
		...
		Request request = new Request(session, story.getGraphPath(appNamespace),
				story.getActionBundle(), HttpMethod.POST, new Request.Callback()
				{
					@Override
					public void onCompleted(Response response)
					{
	...
	private static void publishImpl(Photo photo, String albumId,
			final OnPublishListener onPublishListener)
	{
		Session session = getOpenSession();
		Request request = new Request(session, albumId + "/photos", photo.getBundle(),
	...
	{
		Session session = getOpenSession();
		Request request = new Request(session, "me/videos", video.getBundle(), HttpMethod.POST,
				new Request.Callback()
				{
					@Override
					public void onCompleted(Response response)
	...
	private void openInviteDialog(Activity activity, Bundle params,
			final OnInviteListener onInviteListener)
	{
	...
	private void reopenSession()
	{
		...
		if (session != null && session.getState().equals(SessionState.CREATED_TOKEN_LOADED))
		{
			...
			if (publishPermissions != null && publishPermissions.size() > 0
					&& permissions.containsAll(publishPermissions))
			{
	...
	{
		Session session = Session.getActiveSession();
		Session.NewPermissionsRequest request = new Session.NewPermissionsRequest(mActivity,
				mConfiguration.getPublishPermissions());
	...
	private class SessionStatusCallback implements Session.StatusCallback
	{
		...
		public void call(Session session, SessionState state, Exception exception)
		{
			...
			// log
			logInfo("SessionStatusCallback: state=" + state.name() + ", session="
	...
	{
		/**
		 * If user performed {@link FacebookTools#logout()} action, this
		 * callback method will be invoked
		 */
		void onLogout();
	}
	...
	/**
	 * On permission listener - If the App must request a specific permission
	 * (only to obtain the new Access Token)
	 * 
	 * @author Gryzor
	 * 
	 */
