...
import eoc.studio.voicecard.newspaper.DrawableProcess;
...
public class CardEditorActivity extends BaseActivity {
    ...
    private void initCardDir(int cardId) {
        ...
        File appDir = new File(dir, "eoc.studio.voicecard");
        if (!appDir.exists()) {
            appDir.mkdir();
        }
    ...
    private void restoreUserData(Bundle savedInstanceState) {
        ...
        userSignPositionInfoUri = savedInstanceState.getParcelable(EXTRA_KEY_USER_SIGN_POSITION_INFO);
    ...
    private void saveUserData(Bundle savedInstanceState) {
        ...
        if (userSignHandwritingUri != null) {
            savedInstanceState.putParcelable(EXTRA_KEY_USER_SIGN_HANDWRITHING, userSignHandwritingUri);
            Log.d(TAG, "save user data -- SIGN HANDWRITHING: " + userSignHandwritingUri);
        }

        if (userSignPositionInfoUri != null) {
            savedInstanceState.putParcelable(EXTRA_KEY_USER_SIGN_POSITION_INFO, userSignPositionInfoUri);
            Log.d(TAG, "save user data -- SIGN POSITION: " + userSignPositionInfoUri);
        }

        if (userSignDraftImageUri != null) {
            savedInstanceState.putParcelable(EXTRA_KEY_USER_SIGN_DRAFT_IMAGE, userSignDraftImageUri);
    ...
    private void setCardColor() {
        ...
        GradientDrawable dashBorderBackground = ((GradientDrawable) editableImageFrame.getBackground());
        ...
        ((GradientDrawable) editableImageFrame.getBackground()).setStroke(width, EDIT_COLOR, dashWidth, dashGap);
        ((GradientDrawable) editableVoiceFrame.getBackground()).setStroke(width, EDIT_COLOR, dashWidth, dashGap);
        ((GradientDrawable) editableTextFrame.getBackground()).setStroke(width, EDIT_COLOR, dashWidth, dashGap);
        ((GradientDrawable) editableSignatureFrame.getBackground()).setStroke(width, EDIT_COLOR, dashWidth, dashGap);
        ...
        next.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (isCardUserDataCompleted()) {
                    progressDialog = ProgressDialog.show(CardEditorActivity.this, getString(R.string.processing),
                            getString(R.string.please_wait), true, false);
                    startScreenshotThread();
                } else {
                    Toast.makeText(CardEditorActivity.this, getString(R.string.data_incomplete), Toast.LENGTH_LONG)
                            .show();
                }
            }
        ...
        saveDraft.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                boolean isSuccess = cardDraftManager.saveDraft(new CardDraft(card.getId(), userVoice,
                        userVoiceDuration, userImage, userTextContent, userTextColor, userTextSizeType,
                        userSignHandwritingUri, userSignPositionInfoUri, userSignDraftImageUri));

                if (isSuccess) {
                    Toast.makeText(CardEditorActivity.this, getString(R.string.save_draft_success), Toast.LENGTH_LONG)
                            .show();
                } else {
                    Toast.makeText(CardEditorActivity.this, getString(R.string.save_draft_fail), Toast.LENGTH_LONG)
                            .show();
                }
            }
        ...
        openDraft.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    Log.d(TAG, "openDraft - onClick() user data -- VOICE DURATION: " + userVoiceDuration);
                    ...
                    Log.d(TAG, "openDraft - onClick() user data -- SIGN HANDWRITHING: " + userSignHandwritingUri);
                    Log.d(TAG, "openDraft - onClick() user data -- SIGN POSITION: " + userSignPositionInfoUri);
                    Log.d(TAG, "openDraft - onClick() user data -- SIGN IAMGE: " + userSignDraftImageUri);
                    Toast.makeText(CardEditorActivity.this, getString(R.string.open_draft_success), Toast.LENGTH_LONG)
                            .show();
                } catch (Exception e) {
                    Log.d(TAG, "openDraft - openDraft error:" + e.toString());
                    Toast.makeText(CardEditorActivity.this, getString(R.string.open_draft_fail), Toast.LENGTH_LONG)
                            .show();
                }
...
//	}
    ...
    public void startImageCropper() {
        ...
        int w = editableImage.getWidth();
        int h = editableImage.getHeight();
        DrawableProcess.getImage(this, REQ_CROP_IMAGE, tmpImageUri, w, h);
    }
    ...
    private void onImageCropperResult(int resultCode, Intent data) {
        ...
        if (extras != null) {
            Log.d(TAG, "onImageCropperResult");
            progressDialog = ProgressDialog.show(CardEditorActivity.this, getString(R.string.processing),
                    getString(R.string.please_wait), true, false);

            userImage = tmpImageUri;
            card.setImage(userImage);
    ...
    private void startTextEditor() {
        ...
        intent.putExtra(CardTextEditorActivity.EXTRA_KEY_TEXT_SIZE_TYPE, card.getMessageTextSizeType());
    ...
    private void onTextEditorResult(int resultCode, Intent data) {
        ...
        userTextColor = data.getIntExtra(CardTextEditorActivity.EXTRA_KEY_TEXT_COLOR, Card.DEFAULT_TEXT_COLOR);
...
//            break;
    ...
    private void startCardSender() {
        ...
        intent.putExtra(CardViewerActivity.EXTRA_KEY_CARD_LEFT_SCREENSHOT_FILEPATH, screenshotLeftFilePath);
        intent.putExtra(CardViewerActivity.EXTRA_KEY_CARD_RIGHT_SCREENSHOT_FILEPATH, screenshotRightFilePath);
